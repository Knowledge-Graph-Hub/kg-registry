...
4. Expand or correct the YAML front matter.
   ...
   - Foreign products on a Resource page are expected. They are regenerated by the site build from provenance relationships and are used to show where a resource's data appears in downstream products.
   - Do not unilaterally remove foreign products during curation just because they are not owned by the parent resource. Only remove a product when you have resource-specific evidence that the entry itself is wrong, duplicated, or obsolete.
