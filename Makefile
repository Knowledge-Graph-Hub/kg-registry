...
license-report: reports/license-inference.tsv

# The report is regenerated by `make all` so its conflict rows track the pages.
reports/license-inference.tsv: $(RESOURCES) util/license_inference.py util/infer_licenses.py | reports
	$(RUN) python util/infer_licenses.py --dry-run --report $@
