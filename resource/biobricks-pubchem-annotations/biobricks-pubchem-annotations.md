---
activity_status: active
category: KnowledgeGraph
collection:
- okn
contacts:
- category: Individual
  label: Tom Luechtefeld
  contact_details:
  - contact_type: email
    value: tom@insilica.co
  - contact_type: github
    value: tomlue
- category: Individual
  contact_details:
  - contact_type: email
    value: tom@insilica.co
  label: Tom Luechtefeld
description: BioBricks PubChem Annotations is an open knowledge graph of chemical
  annotations from PubChem.
domains:
- chemistry and biochemistry
homepage_url: https://github.com/biobricks-ai/pubchem-annotations-kg
id: biobricks-pubchem-annotations
layout: resource_detail
name: BioBricks PubChem Annotations
products:
- category: ProgrammingInterface
  description: SPARQL endpoint for BioBricks PubChem Annotations
  id: biobricks-pubchem-annotations.sparql
  name: BioBricks PubChem Annotations SPARQL
  original_source:
  - biobricks-pubchem-annotations
  product_url: https://frink.apps.renci.org/biobricks-pubchem-annotations/sparql
- id: biobricks-pubchem-annotations.tpf
  name: BioBricks PubChem Annotations TPF
  description: Triple Pattern Fragments endpoint for BioBricks PubChem Annotations
  category: ProgrammingInterface
  product_url: https://frink.apps.renci.org/ldf/biobricks-pubchem-annotations
  original_source:
  - biobricks-pubchem-annotations
repository: https://github.com/biobricks-ai/pubchem-annotations-kg
creation_date: '2025-12-08T00:00:00Z'
last_modified_date: '2026-03-30T00:00:00Z'
---
BioBricks PubChem Annotations

## Automated Evaluation

- View the automated evaluation: [biobricks-pubchem-annotations automated evaluation](biobricks-pubchem-annotations_eval_automated.html)
