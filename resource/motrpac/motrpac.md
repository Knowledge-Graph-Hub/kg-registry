...
- category: GraphicalInterface
  ...
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://motrpac-data.org/
  repository: https://github.com/MoTrPAC/motrpac-frontend
- category: ProcessProduct
  description: Harmonized RNA-seq data processing pipeline, implemented in the Workflow
    Description Language (WDL) and run on Google Cloud Platform via Cromwell/caper.
    Produces the transcriptomics quantitative results and QC metrics released through
    the Data Hub.
  format: mixed
  id: motrpac.rna-seq-pipeline
  name: MoTrPAC RNA-seq Pipeline
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/motrpac-rna-seq-pipeline
- category: ProcessProduct
  description: MoTrPAC-specific implementation of the ENCODE ATAC-seq pipeline (v1.7.0),
    including configuration generation, QC flagging criteria, post-processing scripts
    for count matrix generation, and genome databases for human hg38 and rat rn6/rn8.
  format: mixed
  id: motrpac.atac-seq-pipeline
  name: MoTrPAC ATAC-seq QC and Analysis Pipeline
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/motrpac-atac-seq-pipeline
- category: ProcessProduct
  description: Reduced Representation Bisulfite Sequencing (RRBS) data processing pipeline,
    implemented in WDL and run on Google Cloud Platform via Cromwell/caper. Produces
    the DNA methylation portion of the epigenomics data.
  format: mixed
  id: motrpac.rrbs-pipeline
  name: MoTrPAC RRBS Pipeline
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/motrpac-rrbs-pipeline
- category: ProcessProduct
  description: Cloud-based mass spectrometry proteomics data analysis pipeline, implemented
    in WDL and run via Cromwell/caper. Supports MS-GF+/PlexedPiper and MaxQuant based
    workflows for the global, phospho, acetyl, and ubiquityl proteomics datasets.
  format: mixed
  id: motrpac.proteomics-pipeline
  name: MoTrPAC Proteomics Pipeline
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/motrpac-proteomics-pipeline
- category: ProcessProduct
  description: Cluster-ready Snakemake pipeline for analyzing alternative splicing
    from bulk RNA-seq data, with conda-managed environments and configurable execution
    on compute clusters.
  format: mixed
  id: motrpac.rna-splicing-pipeline
  name: MoTrPAC RNA Splicing Pipeline
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/motrpac-rna-splicing-pipeline
- category: ProcessProduct
...
- category: ProcessProduct
  description: R package with the analysis code and convenience functions used for
    the MoTrPAC endurance exercise training study in 6-month-old rats, including differential
    analysis, clustering, and pathway enrichment of the multi-omic results.
  format: mixed
  id: motrpac.rat-training-6mo
  license:
    id: https://opensource.org/license/mit/
    label: MIT
  name: MotrpacRatTraining6mo
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/MotrpacRatTraining6mo
- category: Product
  description: R data package distributing the processed data and analysis results
    for the MoTrPAC endurance exercise training study in 6-month-old rats, as versioned
    R objects that accompany the MotrpacRatTraining6mo analysis package.
  format: mixed
  id: motrpac.rat-training-6mo-data
  license:
    id: https://opensource.org/license/mit/
    label: MIT
  name: MotrpacRatTraining6moData
  original_source:
  - relation_type: prov:hadPrimarySource
    source: motrpac
  product_url: https://github.com/MoTrPAC/MotrpacRatTraining6moData
...
MoTrPAC develops its software in the [MoTrPAC GitHub organization](https://github.com/MoTrPAC). The per-assay processing pipelines are implemented in WDL and run on Google Cloud Platform through Cromwell/[caper](https://github.com/MoTrPAC/caper): [motrpac-rna-seq-pipeline](https://github.com/MoTrPAC/motrpac-rna-seq-pipeline) for transcriptomics, [motrpac-atac-seq-pipeline](https://github.com/MoTrPAC/motrpac-atac-seq-pipeline) (a MoTrPAC-specific implementation of the ENCODE ATAC-seq pipeline) and [motrpac-rrbs-pipeline](https://github.com/MoTrPAC/motrpac-rrbs-pipeline) for epigenomics, and [motrpac-proteomics-pipeline](https://github.com/MoTrPAC/motrpac-proteomics-pipeline) for untargeted proteomics. Alternative splicing is analyzed with the Snakemake workflow in [motrpac-rna-splicing-pipeline](https://github.com/MoTrPAC/motrpac-rna-splicing-pipeline).

QC/QA for the Chemical Analysis Site submissions is provided by the [MotrpacBicQC](https://github.com/MoTrPAC/MotrpacBicQC) R package. Downstream analysis of the young adult rat endurance training study is distributed as a pair of R packages: [MotrpacRatTraining6mo](https://github.com/MoTrPAC/MotrpacRatTraining6mo) (analysis code) and [MotrpacRatTraining6moData](https://github.com/MoTrPAC/MotrpacRatTraining6moData) (the accompanying data objects). The Data Hub website itself is built from [motrpac-frontend](https://github.com/MoTrPAC/motrpac-frontend).
