...
function initializeGraph(data) {
  ...
  // Add event listeners for switching renderers
  document.getElementById('use-d3').addEventListener('click', () => switchRenderer('D3'));
  document.getElementById('use-pixi').addEventListener('click', () => switchRenderer('PixiJS'));
}
...
function switchRenderer(rendererType) {
  ...
  if (rendererType === 'D3') {
    ...
    // Check if we can use D3 only mode instead
    if (!window.PIXI) {
      alert('The PixiJS library could not be loaded. Using D3 renderer instead.');
      switchRenderer('D3');
      return;
    }
...
// ===== PIXIJS VIEWPORT LIBRARY INLINE =====
// This is a minimal implementation of the viewport functionality we need
class SimpleViewport extends PIXI.Container {
  constructor(options) {
    super();
    this.screenWidth = options.screenWidth || 800;
    this.screenHeight = options.screenHeight || 600;
    this.worldWidth = options.worldWidth || this.screenWidth * 2;
    this.worldHeight = options.worldHeight || this.screenHeight * 2;
  ...
  pinch() {
    // We're not implementing pinch zoom for this simplified version
    return this;
  }
  
  decelerate() {
    // We're not implementing deceleration for this simplified version
    return this;
  }
  ...
  _onDragStart(event) {
    this._dragging = true;
    this._lastViewportX = event.data.global.x;
    this._lastViewportY = event.data.global.y;
  }
  
  _onDragMove(event) {
    if (this._dragging) {
      const dx = event.data.global.x - this._lastViewportX;
      const dy = event.data.global.y - this._lastViewportY;
  ...
  // Wheel zoom handler
  _onWheel(event) {
    // Only handle if mouse is over the viewport
    const bounds = this.interaction.renderer.view.getBoundingClientRect();
    ...
    if (x >= 0 && x <= bounds.width && y >= 0 && y <= bounds.height) {
      ...
      const zoomFactor = delta > 0 ? (1 + this._wheelZoom) : (1 - this._wheelZoom);
      ...
      this.position.x = x - worldPos.x * this.scale.x;
      this.position.y = y - worldPos.y * this.scale.y;
    }
  }
}
...
function setupPixiVisualization() {
  ...
  try {
    // Use our own SimpleViewport implementation if the library is not available
    // Check if Viewport is available externally
    const ViewportClass = window.Viewport || SimpleViewport;
    console.log('Using ViewportClass:', ViewportClass.name || 'Custom');
    ...
    alert('Error setting up PixiJS visualization. Switching to D3 renderer.');
    switchRenderer('D3');
  }
}
...
function onNodeHoverOut() {
  try {
    ...
    console.error('Error in onNodeHoverOut:', error);
  }
}
...
function highlightConnectionsPixi(node) {
  try {
    ...
    if (selectedIndex >= 0 && pixiNodes[selectedIndex]) {
    ...
    // Highlight connected nodes
    graph.nodes.forEach((n, i) => {
      if (connectedNodeIds.has(n.id) && pixiNodes[i]) {
        pixiNodes[i].alpha = 1;
      }
    });
    ...
    graph.links.forEach((l, i) => {
      if ((l.source.id === node.id || l.target.id === node.id) && pixiLinks[i]) {
    ...
    console.error('Error in highlightConnectionsPixi:', error);
  }
}
...
function resetHighlightingPixi() {
  try {
    ...
    console.error('Error in resetHighlightingPixi:', error);
  }
}
