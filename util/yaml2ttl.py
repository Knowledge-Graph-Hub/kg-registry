...
using `rdflib`. For each resource entry, it creates a URI using the KG-Registry namespace
(kgr: https://w3id.org/kg-registry/) and maps a comprehensive set of properties to RDF
predicates using standard namespaces (Dublin Core, Schema.org, FOAF, SKOS, etc.).

Namespaces used:
    - kgr: https://w3id.org/kg-registry/ (KG-Registry resources)
...
Properties mapped include:
    ...
    - category (rdf:type)
    - domains, tags, taxon, synonyms (multi-valued)
    - creation_date, last_modified_date (dates with xsd:dateTime)
    - contacts, curators (FOAF relationships)
    - products (with schema:hasPart)
    - funding, publications (nested properties)
...
def main(argv):
    ...
    g.bind("kgr", Namespace(KGREGISTRY_BASE))
