...
"""
Parallel version of retrieve-file-sizes.py for faster URL checking.

This version uses concurrent.futures.ThreadPoolExecutor to check multiple URLs
in parallel, significantly speeding up the build process.
"""
...
def load_url_cache(cache_path: pathlib.Path) -> Dict[str, Any]:
    if cache_path.exists():
        try:
            ...
            safe_print(f"⚠️  Could not read cache at {cache_path}: {e}")
    return {}
...
def save_url_cache(cache: Dict[str, Any], cache_path: pathlib.Path) -> None:
    try:
        ...
        safe_print(f"💾 URL status cache written: {cache_path} ({len(cache)} entries)")
    except Exception as e:
        safe_print(f"⚠️  Could not write cache file {cache_path}: {e}")
...
def get_file_size_from_header(url: str) -> Tuple[Optional[int], Optional[str], Dict[str, Any]]:
    ...
    try:
        ...
        if url != original_url:
            ...
            safe_print(f"  🔄 Converted to raw URL: {url}")
        else:
        ...
        if 'text/html' in ct_lower:
            safe_print(f"  ⏭️  Skipping HTML page (Content-Type: {ct_lower})")
        ...
        if not content_type:
            ...
            safe_print("  ⏭️  No Content-Type header; will cache skip for future runs")
        ...
        try:
            ...
            safe_print(f"  ✅ File size: {format_file_size(file_size)}")
            info['content_length'] = file_size
            return file_size, None, info
        except ValueError:
...
def process_single_url(url: str, product_id: str, resource_id: str, cache: Dict[str, Any],
    ...
    if should_skip_cache:
        ...
        if has_retrieval_warning:
            safe_print(f"🔁 Re-checking cached URL to clear potential stale warning: {url}")
        else:
            reason = cache_entry.get('skip_reason') if isinstance(cache_entry, dict) else 'cached'
...
def write_file_sizes_to_resource_files(updated_products: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    if not updated_products:
        safe_print("No file sizes to write back to resource files")
        return
    ...
    for resource_id, products in updated_products.items():
        ...
        try:
            ...
            if not pathlib.Path(resource_file).exists():
                safe_print(f"  ⚠️  Resource file not found: {resource_file}")
                continue
            ...
            if updated_count > 0:
                ...
                safe_print(f"  ✅ Updated {updated_count} products in {resource_file}")
            else:
                safe_print(f"  ℹ️  No updates needed for {resource_file}")
...
def update_product_file_sizes(data: Dict[str, Any], limit: Optional[int] = None, *, cache: Dict[str, Any],
    ...
    """
    Update product_file_size for all products in the registry data using parallel processing.

    Args:
        data: Registry data dictionary
        limit: Optional limit on number of products to process (for testing)
        cache: URL status cache dictionary
        ignore_cache: Whether to ignore existing cache entries
        clean_warnings_only: Only clear warnings without making network requests
        max_workers: Number of parallel workers for URL checking

    Returns:
        Tuple of (updated registry data, dictionary of updated products by resource_id)
    """
    if 'resources' not in data:
        safe_print("No resources found in data")
        return data, {}
    ...
    for resource in data['resources']:
        ...
        for product in resource['products']:
            ...
            if limit is not None and processed_products >= limit:
                safe_print(f"🔬 Reached limit of {limit} products, stopping")
                break
    ...
    safe_print(f"   Failed: {failed_products}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Retrieve file sizes for products with URLs (PARALLEL VERSION)",
    ...
    parser.add_argument("--max-workers", type=int, default=DEFAULT_MAX_WORKERS,
                        help=f"Number of parallel workers for URL checking (default: {DEFAULT_MAX_WORKERS})")
    ...
    try:
        ...
        safe_print(f"Error loading input file {args.input_file}: {e}")
        sys.exit(1)

    if args.dry_run:
        safe_print("🔍 DRY RUN MODE - No changes will be saved")

    if args.limit:
        safe_print(f"🔬 LIMIT MODE - Processing only first {args.limit} products")

    safe_print(f"⚡ PARALLEL MODE - Using {args.max_workers} workers")

    # Load cache
    cache_path = pathlib.Path(args.cache_file)
    cache = load_url_cache(cache_path)
    if cache:
        safe_print(f"🗃️  Loaded URL status cache with {len(cache)} entries from {cache_path}")
    else:
        safe_print(f"🗃️  Starting with empty URL status cache (path: {cache_path})")
    ...
    if not args.dry_run:
        ...
        try:
            ...
            safe_print(f"✅ Updated data saved to {args.output_file}")
        except Exception as e:
            safe_print(f"Error saving output file {args.output_file}: {e}")
            sys.exit(1)
        # Persist cache
        save_url_cache(cache, cache_path)
    else:
        safe_print("🔍 Dry run completed - no files were modified")
